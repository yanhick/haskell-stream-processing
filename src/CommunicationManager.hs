...
data CommunicationManagerMessage = TaskResult TaskId ByteString | PartitionResult TaskId ProcessId ByteString
  deriving (Generic, Show)

instance Binary CommunicationManagerMessage where

newtype IntermediateResult = IntermediateResult ByteString
  deriving (Generic)
